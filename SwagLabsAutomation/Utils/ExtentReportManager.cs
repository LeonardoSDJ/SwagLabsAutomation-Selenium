...
public abstract class ExtentReportManager
{
    ...
    public static AventStack.ExtentReports.ExtentReports GetInstance()
    {
        try
        {
            ...
            _extent.AddSystemInfo("Data/Hora", DateTime.Now.ToString(CultureInfo.InvariantCulture));
    ...
    public static void EndReport()
    {
        try
        {
            Console.WriteLine(value: "Finalizando relatório...");
            if (_extent != null)
            {
                _extent.Flush();
                Console.WriteLine(value: $"Relatório gerado em: {Path.GetFullPath(path: _reportPath ?? throw new InvalidOperationException())}");
            }
            else
            {
                Console.WriteLine(value: "AVISO: _extent é nulo, não foi possível finalizar o relatório!");
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine(value: $"ERRO ao finalizar relatório: {ex.Message}");
